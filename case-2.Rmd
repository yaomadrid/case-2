...
```{r}
fmla2<-Rocky5 ~ Rocky1 + Rocky2 + Rocky3 + Rocky4 + isMissing1 + isMissing2 + isMissing3 + isMissing4
...
mselist<-list()
for (i in 1:16) {
modellist2[[i]]<-(lm(as.formula(formulaSet[i]),data=valid.df))
mselist[[i]]<-mse(modellist2[[i]])
}
